...
TShape *sp[MAX_NODE];                           //节点Shap数组
TLabel *lb[MAX_NODE];                           //节点Shap上的标签数组，画板->Shap->Label
int graph[MAX_NODE][MAX_NODE] = { 9999 };       //存储图的矩阵结构 graph[i][i] = n:代表第i个节点到第j个节点的距离是n

int direct = 1;                                 //默认是有向图

int x[MAX_NODE], y[MAX_NODE];                   //存储图在屏幕的坐标位置
int n;                                          //n:记录节点总数
...
//---------------------------------------------------------------------------
void __fastcall TForm2::CreateNodeBtClick(TObject * Sender)    //生成顶点按钮单击事件
{
...
void __fastcall TForm2::imgMouseDown(TObject * Sender, //鼠标按下事件,记录起始点的位置
        TMouseButton Button,
        TShiftState Shift, int X, int Y)
{
    if (CreateNodeBt->Enabled == true && input_node == true) {
        if (n < MAX_NODE) {    //限制最多输入MAX_NODE个节点
            int tmp = overLap(X, Y);
            if (tmp != -1) {
                ShowMessage("与节点:" + IntToStr(tmp) + "相邻太近");
                return;
            }
            sp[n] = new TShape(Form2);    //建立新结点控件
...
//---------------------------------------------------------------------------
void __fastcall TForm2::ShortestPathBtClick(TObject * Sender)    //最短路径求解事件
{
    bool vst[MAX_NODE] = { false };     //访问辅助数组
    int dte[MAX_NODE] = { 0 };          //单源最短路径算法存储最短路径dte[i] = x 代表从节点0到节点i的最短路径是x
    int target = -1;
    if (n == 0) {
...
//---------------------------------------------------------------------------
void __fastcall TForm2::FormCreate(TObject * Sender)    //顶点间初始设置为无穷大(不通)
{
