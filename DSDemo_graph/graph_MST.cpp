...
//---------------------------------------------------------------------------
__fastcall TGraphMSTForm::TGraphMSTForm(TComponent * Owner):TForm(Owner)
{
}

//---------------------------------------------------------------------------
void __fastcall TGraphMSTForm::FormCreate(TObject * Sender)    //顶点间初始设置为无穷大(不通)
{
    memo_local = memo;
...
//---------------------------------------------------------------------------
void __fastcall TGraphMSTForm::CreateNodeBtnClick(TObject * Sender)    //生成顶点按钮单击事件
{
...
void __fastcall TGraphMSTForm::imgMouseDown(TObject * Sender, //鼠标按下事件,记录起始点的位置
        TMouseButton Button,
...
void __fastcall TGraphMSTForm::imgMouseUp(TObject * Sender,
        TMouseButton Button,
        TShiftState Shift, int X, int Y)
{
    int dist = 0;
...
void __fastcall TGraphMSTForm::my_print (String info)
{
    memo->Lines->Add(info);
}


void __fastcall TGraphMSTForm::BtClearMemoClick(TObject * Sender)
{
    memo->Clear();
}
...
void __fastcall TGraphMSTForm::ResetBtnClick(TObject * Sender)
{
    int i;
...
{
 memo->Lines->Add("用Prim算法生成最小生成树结果如下:");
 MG_Prim_MST(&MG, 0);
}
